	...
	public partial class FindAndReplaceForm : Form
	{
		...
		public void ShowFor(TextEditorControl editor, bool replaceMode)
		{
			...
			if (sm.HasSomethingSelected && sm.SelectionCollection.Count == 1) {
				...
				//if (sel.StartPosition.Line == sel.EndPosition.Line) //tx
				if (sel.StartPosition.X == sel.EndPosition.X)
...
*/
			Point p1 = _editor.Document.OffsetToPosition(range.Offset);
            Point p2 = _editor.Document.OffsetToPosition(range.Offset + range.Length);
			_editor.ActiveTextAreaControl.SelectionManager.SetSelection(p1, p2);
			//_editor.ActiveTextAreaControl.ScrollTo(p1.Line, p1.Column); //tx
			_editor.ActiveTextAreaControl.ScrollTo(p1.X, p1.Y);
		...
		private void btnReplaceAll_Click(object sender, EventArgs e)
		{
			...
			// _editor.Document.UndoStack.StartUndoGroup(); //tx
			try {
            ...
            {
                // _editor.Document.UndoStack.EndUndoGroup(); //tx
			}
			if (count == 0)
		...
		private void InsertText(string text)
		{
            ...
            // textArea.Document.UndoStack.StartUndoGroup(); //tx
			try {
            ...
            {
                //textArea.Document.UndoStack.EndUndoGroup(); //tx
			}
		}
